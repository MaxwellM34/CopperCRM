...
def main(sender: str, index: int = 0):
    ...
    latest_id = ids[-1]  # last ID = most recent email (negative indexing is cool)
    logging.info(f"Fetching latest email ID {latest_id.decode()}...")

    status, msg_data = mail.fetch(latest_id, "(RFC822)") # fetch the email we found
