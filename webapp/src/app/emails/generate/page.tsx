...
export default function EmailGeneratorPage() {
  ...
  return (
    <AppShell title="Generate Emails" subtitle="Create first-touch emails for unmessaged leads.">
      <section className="glass rounded-2xl p-6">
        <div className="grid gap-4 sm:grid-cols-3">
          <Stat title="Pending" value={loadingStats ? "…" : pending} hint="Leads without a first email." />
          <Stat title="Generated" value={loadingStats ? "…" : stats?.generated ?? 0} hint="Already created." />
          <Stat
            title="Est. Total"
            value={loadingStats ? "…" : `$${(stats?.estimated_total_cost_usd ?? 0).toFixed(4)}`}
            hint={
              stats?.sample_size
                ? `Based on ${stats.sample_size} samples`
      ...
      <section className="glass rounded-2xl p-6">
        ...
        {lastRun ? (
          <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <Stat title="Attempted" value={lastRun.attempted} />
            <Stat title="Generated" value={lastRun.generated} />
            <Stat title="Pending now" value={lastRun.pending_after} />
            <Stat
              title="Cost"
